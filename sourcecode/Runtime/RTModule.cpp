...
namespace Nom
{
	namespace Runtime
	{
		...
		RTModule::RTModule(NomModule* mod) : theModule(std::make_unique<llvm::Module>("module", LLVMCONTEXT))
		{
			...
			/*
			std::string CMethodName = "foo1414";
			std::string className = "InstanceTest_1";
			std::string callMethodName = "testInst1";
			...
			NomTypeParameterRef ref1 = new NomTypeParameterInternal(NULL, 0, NomType::AnythingRef, NomType::NothingRef);
			std::vector<NomTypeParameterRef> typeArgsArray = {ref, ref1};
			std::vector<NomTypeRef> argTypesArray = {ref->GetVariable(), ref1->GetVariable()};
			llvm::ArrayRef<NomTypeRef> argTypes = llvm::ArrayRef<NomTypeRef>(argTypesArray);
			llvm::ArrayRef<NomTypeParameterRef> typeArgs = llvm::ArrayRef<NomTypeParameterRef>(typeArgsArray);
			NomTypeRef returnType = NomStringClass::GetInstance()->GetType();
			*/

			/*
			std::string CMethodName = "foo1414";
			std::string className = "Main_0";
			std::string callMethodName = "test8";
			//NomTypeRef returnType = (NomType*)NomBoolClass::GetInstance()->GetType();
			NomTypeParameterRef ref = new NomTypeParameterInternal(NULL, 0, NomType::AnythingRef, NomType::NothingRef);
			NomTypeParameterRef ref1 = new NomTypeParameterInternal(NULL, 0, NomType::AnythingRef, NomType::NothingRef);
			...
			std::vector<NomTypeRef> subTypesArray = {ref->GetVariable()};
			llvm::ArrayRef<NomTypeRef> subTypes = llvm::ArrayRef<NomTypeRef>(subTypesArray);
			NomSubstitutionContextList list = NomSubstitutionContextList(subTypes);
			NomString IT = NomString("InstanceTest_1");
			NomTypeRef returnType = NomClass::getClass(&IT)->GetType(subTypes);

			std::vector<NomTypeRef> argTypesArray = {NomClass::getClass(&IT)->GetType(subTypes)};
			llvm::ArrayRef<NomTypeRef> argTypes = llvm::ArrayRef<NomTypeRef>(argTypesArray);
			*/
			...
			/*
			std::string CMethodName = "foo1414";
			std::string callMethodName = "testInst";
			std::string className = "InstanceTest_1";
			
			NomTypeParameterRef ref = new NomTypeParameterInternal(NULL, 0, NomType::AnythingRef, NomType::NothingRef);
			std::vector<NomTypeParameterRef> typeArgsArray = {};
			std::vector<NomTypeParameterRef> recTypeArgsArray = {ref};
			std::vector<NomTypeRef> argTypesArray = {NomStringClass::GetInstance()->GetType(), NomIntClass::GetInstance()->GetType()};

			llvm::ArrayRef<NomTypeRef> argTypes = llvm::ArrayRef<NomTypeRef>(argTypesArray);
			llvm::ArrayRef<NomTypeParameterRef> typeArgs = llvm::ArrayRef<NomTypeParameterRef>(typeArgsArray);
			llvm::ArrayRef<NomTypeParameterRef> recTypeArgs = llvm::ArrayRef<NomTypeParameterRef>(recTypeArgsArray);


			std::vector<NomTypeRef> lookupArray = {ref->GetVariable()};
			llvm::ArrayRef<NomTypeRef> lookup = llvm::ArrayRef<NomTypeRef>(lookupArray);
			NomClassTypeRef recType = NomClass::getClass(new NomString(className))->GetType(lookup);
			...
			*/
			...
			std::string CMethodName = "foo1414";
			std::string className = "InstanceTest_1";
			//NomClassTypeRef recType = NomStringClass::GetInstance()->GetType();


			NomTypeParameterRef ref = new NomTypeParameterInternal(NULL, 0, NomType::AnythingRef, NomType::NothingRef);
			std::vector<NomTypeParameterRef> typeArgsArray = {ref};
			std::vector<NomTypeRef> argTypesArray = {};

			llvm::ArrayRef<NomTypeRef> argTypes = llvm::ArrayRef<NomTypeRef>(argTypesArray);
			llvm::ArrayRef<NomTypeParameterRef> typeArgs = llvm::ArrayRef<NomTypeParameterRef>(typeArgsArray);
			llvm::ArrayRef<NomTypeParameterRef> recTypeArgs = llvm::ArrayRef<NomTypeParameterRef>(recTypeArgsArray);

			std::vector<NomTypeRef> lookupArray = {};
			llvm::ArrayRef<NomTypeRef> lookup = llvm::ArrayRef<NomTypeRef>(lookupArray);
			NomClassTypeRef recType = NomStringClass::GetInstance()->GetType();

			NomTypeRef returnType = NomVoidClass::GetInstance()->GetType();
			*/


			//NomCLib* staticLib = new NomCLibConstructor(CMethodName,nir, argTypes,typeArgs,returnType);
			//staticLib->GetLLVMElement(*theModule.get());

			SpecParser spec = SpecParser();
			int i = spec.functions.size();
			for (GeneratedFunction* func : spec.functions) {
				func->generatedLibrary->GetLLVMElement(*theModule.get());
			}
