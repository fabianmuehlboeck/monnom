...
namespace Nom
{
	namespace Runtime
	{
		llvm::Value* PWLambda::ReadField(NomBuilder& builder, int32_t fieldIndex)
		{
		...
		llvm::Value* PWLambda::WriteField(NomBuilder& builder, int32_t fieldIndex, llvm::Value* value)
		{
		...
		llvm::Value* PWLambdaPrecise::ReadLambdaField(NomBuilder& builder, int32_t fieldIndex)
		{
			return MakeInvariantLoad(builder, REFTYPE, builder->CreateGEP(LambdaHeader::GetLLVMType(), wrapped, { MakeInt32(0), MakeInt32(LambdaHeaderFields::Fields), MakeInt32(fieldIndex) }));
		}
	}
