...
namespace Nom
{
	namespace Runtime
	{
		/*void GeneratePrimitiveTypeCase(NomBuilder& builder, NomValue &value, []void(*handleInt)(NomBuilder& builder, NomValue&, NomValue*), []void(*handleFloat)(NomBuilder& builder, NomValue&, NomValue*), []void(*handleRef)(NomBuilder& builder, NomValue&, llvm::Value*, NomValue*), NomValue* returnValues, int numReturns)
		{
			...
			if (intPossible)
			{
				...
				auto extractInt = builder->CreatePtrToInt(ObjectHeader::ReadField(builder, value, MakeInt32(0)), INTTYPE, "unboxedInt");
				...
				auto posFsh = builder->CreateIntrinsic(Intrinsic::fshr, { prefunnel, prefunnel, ConstantInt::get(prefunnel->getType(), 3) });
				builder->CreateBr(intBlock);

				builder->SetInsertPoint(negintBlock);
				auto negFsh = builder->CreateIntrinsic(Intrinsic::fshr, { refAsInt, refAsInt, ConstantInt::get(refAsInt->getType(), 3) });
				builder->CreateBr(intBlock);
			...
			if (floatPossible)
			{
				...
				auto boxField = builder->CreateBitCast(builder->CreatePtrToInt(ObjectHeader::ReadField(builder, value, MakeInt32(0)), INTTYPE), FLOATTYPE, "unboxedFloat");
				...
				auto unmaskedFloat = builder->CreateBitCast(builder->CreateIntrinsic(Intrinsic::fshr, { refAsInt, refAsInt, ConstantInt::get(refAsInt->getType(), 3) }), FLOATTYPE, "unmaskedFloat");
				builder->CreateBr(floatBlock);
			...
			for (int i = 0; i < numReturns; i++)
			{
				...
				if (intPossible)
				{
					...
					returnType = returnType->Join(intReturns[i].GetNomType());
					isFCall = isFCall && intReturns[i].IsFunctionCall();
				}
				if (floatPossible)
				{
					returnPHI->addIncoming(floatReturns[i], floatBlock);
					returnType = returnType->Join(floatReturns[i].GetNomType());
