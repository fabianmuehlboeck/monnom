...
namespace Nom
{
	namespace Runtime
	{
		...
		void EnsureDynamicMethodInstruction::Compile(NomBuilder& builder, CompileEnv* env, int lineno)
		{
			...
			//auto methodName = NomConstants::GetString(MethodName)->GetText()->ToStdString();
			//std::string* invalidMethodErrorMsg = new std::string("Invalid method name for given object: " + methodName);
			//BasicBlock* invalidMethodErrorBlock = RTOutput_Fail::GenerateFailOutputBlock(builder, invalidMethodErrorMsg->c_str());
			//BasicBlock* returnBlock = BasicBlock::Create(LLVMCONTEXT, "ensureMethodOut", builder->GetInsertBlock()->getParent());

			//if (refValueBlock != nullptr)
			//{
			//	builder->SetInsertPoint(refValueBlock);
			//	auto vTable = RefValueHeader::GenerateReadVTablePointer(builder, receiver);
			//	auto methodEnsureFunction = RTInterface::GenerateReadMethodEnsure(builder, vTable);
			//	auto ensureMethodFunctionCall = builder->CreateCall(GetMethodEnsureFunctionType(), methodEnsureFunction, { receiver, MakeInt<size_t>(NomNameRepository::Instance().GetNameID(methodName)) });
			//	ensureMethodFunctionCall->setCallingConv(NOMCC);
			//	builder->CreateCondBr(ensureMethodFunctionCall, returnBlock, invalidMethodErrorBlock);
			//}
			...
			//	bool found = false;
			//	for (auto& meth : NomIntClass::GetInstance()->MethodTable)
			//	{
			//		if (methodName == meth->Method->GetName())
			//		{
			//			found = true;
			//			break;
			//		}
			...
			//	else
			//	{
			//		builder->CreateBr(invalidMethodErrorBlock);
			//	}
			//}
			...
			//	bool found = false;
			//	for (auto& meth : NomFloatClass::GetInstance()->MethodTable)
			//	{
			//		if (methodName == meth->Method->GetName())
			//		{
			//			found = true;
			...
			//	builder->SetInsertPoint(primitiveBoolBlock);
			//	bool found = false;
			//	for (auto& meth : NomBoolClass::GetInstance()->MethodTable)
			//	{
			//		if (methodName == meth->Method->GetName())
			//		{
			//			found = true;
