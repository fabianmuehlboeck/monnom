...
namespace Nom
{
	namespace Runtime
	{
		...
		llvm::Constant* NomBoolObjects::GetTrue(llvm::Module& mod)
		{
			...
			return llvm::ConstantExpr::getPointerCast(llvm::ConstantExpr::getGetElementPtr(GetInstance()->GetLLVMElementType(mod), elem, llvm::ArrayRef<llvm::Constant*>({ {MakeInt(0), MakeInt(1), MakeInt(1)} })), REFTYPE);
		}

		llvm::Constant* NomBoolObjects::GetFalse(llvm::Module& mod)
		{
			auto elem = GetInstance()->GetLLVMElement(mod);
			return llvm::ConstantExpr::getPointerCast(llvm::ConstantExpr::getGetElementPtr(GetInstance()->GetLLVMElementType(mod), elem, llvm::ArrayRef<llvm::Constant*>({ {MakeInt(0), MakeInt(0), MakeInt(1)} })), REFTYPE);
		}
		...
		llvm::Value* NomBoolObjects::PackBool(NomBuilder& builder, llvm::Value* b)
		{
			...
			auto elem = GetInstance()->GetLLVMElement(*mod);
			return builder->CreatePointerCast(builder->CreateGEP(GetInstance()->GetLLVMElementType(*mod), elem, llvm::ArrayRef<llvm::Value*>({{MakeInt(0), index,MakeInt(1)}})), REFTYPE);
		}
		...
		{
			auto var = new llvm::GlobalVariable(mod, GetLLVMElementType(mod), true, linkage, nullptr, "RT_NOM_BOOLEANS");
			auto clsref = NomBoolClass::GetInstance()->GetLLVMElement(mod);
			auto fieldstype = arrtype(REFTYPE, 1);
			llvm::Constant* falseConst = ObjectHeader::GetConstant(clsref, llvm::ConstantArray::get(fieldstype, { llvm::ConstantExpr::getIntToPtr(MakeInt(1, (uint64_t)0), REFTYPE) }), llvm::ConstantArray::get(NLLVMPointerArr(RTTypeHead::GetLLVMType(), 0), {}));
			llvm::Constant* trueConst = ObjectHeader::GetConstant(clsref, llvm::ConstantArray::get(fieldstype, { llvm::ConstantExpr::getIntToPtr(MakeInt(1, (uint64_t)1), REFTYPE) }), llvm::ConstantArray::get(NLLVMPointerArr(RTTypeHead::GetLLVMType(), 0), {}));
		...
		llvm::Type* NomBoolObjects::GetLLVMElementType(llvm::Module& mod) const
		{
