...
namespace Nom
{
	namespace Runtime
	{
		...
		{
			static const llvm::ArrayRef<const NomInterfaceInternal*> GetStdLibInterfaces()
			{
			...
			static const llvm::ArrayRef<const NomClassInternal*> GetStdLibClasses()
			{
		...
		void LocalLibrarySource::GetLibraryVersions(const std::string* name, std::list<LibraryVersion>& versions)
		{
			...
			if (file != nullptr)
			{
				...
				const std::string* _name = &(manifest->Name);
				...
				versions.emplace_back(_name, mainclassname, libVersion, compFromVersion, compToVersion, isDeprecated, isSecurityRisk, std::move(dependencies), new ManifestComponentSource(Path, manifest, lcs.Program));
			}
		...
		const NomInterface* InternalComponentSource::LoadInterface(NomStringRef name, NomModule* mod)
		{
			...
			{
				size_t targcount = std::stoul(name->ToStdString().substr(4));
