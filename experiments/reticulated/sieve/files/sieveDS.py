...
def CountFrom(n):
    return check1(Stream(n, (lambda : CountFrom((n + 1)))))
...
def Sift(n, s):
    ...
    f = check2(s).N
    while ((f % n) == 0):
        s = check_type_function(check3(s).Rest)()
        f = check2(s).N
    return check1(Stream(f, (lambda : Sift(n, check_type_function(check3(s).Rest)()))))
Sift = check_type_function(Sift)

def Sieve(s):
    return check1(Stream(check2(s).N, (lambda : Sieve(Sift(check_type_int(check2(s).N), check_type_function(check3(s).Rest)())))))
...
if (__name__ == '__main__'):
    parser = check_type_function(check4(optparse).OptionParser)(usage='%prog [options]', description='Test the performance of the Sieve benchmark')
